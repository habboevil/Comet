...
public class PermissionsDao {
        ...
        try {
            ...
            while (resultSet.next()) {
                data.putIfAbsent(resultSet.getInt("id"), new Rank(
                        ...
                        resultSet.getString("alfa_tool").equals("1"),
                        ...
                        resultSet.getString("messenger_alfa_chat").equals("1"),
    ...
    public static Map<String, CommandPermission> getCommandPermissions() {
        ...
        try {
            sqlConnection = SqlHelper.getConnection();
            preparedStatement = SqlHelper.prepare("SELECT `command_id`, `minimum_rank`, `vip_only`, `rights_only`, `rights_override` FROM permission_commands", sqlConnection);
            resultSet = preparedStatement.executeQuery();
    ...
    public static Map<String, OverrideCommandPermission> getOverrideCommandPermissions() {
        ...
        try {
            sqlConnection = SqlHelper.getConnection();
            preparedStatement = SqlHelper.prepare("SELECT `id`, `command_id`, `player_id`, `rank_id`, `enabled` FROM permission_command_overrides", sqlConnection);
            resultSet = preparedStatement.executeQuery();
