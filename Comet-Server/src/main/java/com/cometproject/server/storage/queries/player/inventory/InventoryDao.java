...
public class InventoryDao {
    private static final Logger log = LogManager.getLogger(InventoryDao.class.getName());
    ...
    public static Map<Long, PlayerItem> getInventoryByPlayerId(int playerId) {
        ...
        final Map<Long, PlayerItem> data = new HashMap<>();
        ...
        try {
            ...
            while (resultSet.next()) {
                final PlayerItem playerItem = new InventoryItem(resultSet);
    ...
    public static String[] getWornBadgesByPlayerId(int playerId) {
        ...
        try {
            ...
            preparedStatement = SqlHelper.prepare("SELECT * FROM player_badges WHERE player_id = ? AND slot != 0 LIMIT 10", sqlConnection);
            preparedStatement.setInt(1, playerId);
    ...
    public static void clearInventory(int userId) {
        ...
        try {
            ...
            preparedStatement = SqlHelper.prepare("DELETE FROM items WHERE user_id = ? AND room_id = 0 AND base_item NOT IN (SELECT f.id FROM furniture f WHERE f.deleteable = 0)", sqlConnection);
