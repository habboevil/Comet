    ...
    public void purchase(ICatalogItem item, int itemId, ISession client, int amount, ICatalogPage page, GiftData giftData, FurnitureDefinition definition, ICatalogBundledItem bundledItem, String data) {
        ...
        if (totalDays > 0) {
            if (client.getPlayer().getData().getVipPoints() < totalDiamonds) {
                client.send(new NotificationMessageComposer("generic", "You do not have enough diamonds for this purchase."));
                return;
            }

            if (client.getPlayer().getData().getCredits() < totalCredits) {
                client.send(new NotificationMessageComposer("generic", "\n" + "You do not have enough credits for this purchase."));
                return;
            }
            ...
            client.send(new NotificationMessageComposer("VIP", Locale.getOrDefault("message.vip.buy", "Congratulations, you are now VIP!")));
