...
public class PushCommand extends ChatCommand {
    ...
    public void execute(Session client, String[] params) {
        ...
        if (client.getPlayer().getEntity().isRoomMuted() || client.getPlayer().getEntity().getRoom().getRights().hasMute(client.getPlayer().getId())) {
            sendNotif(Locale.getOrDefault("command.user.muted", "You are on mute."), client);
            return;
        }
        ...
        if (user.getPlayer().getEntity() == null) {
            sendNotif(Locale.getOrDefault("command.user.notinroom", "This user is not in the room."), client);
            return;
        }
    ...
    private void push(RoomEntity entity, Session client) {
        ...
        if (!((Math.abs((posX - playerX)) >= 2) || (Math.abs(posY - playerY) >= 2))) {
            ...
            client.getPlayer().getSession().send(new WhisperMessageComposer(client.getPlayer().getEntity().getId(), Locale.getOrDefault("command.notaround", "Oeps! %playername% is not nearby, walk to this player.").replace("%playername%", entity.getUsername()), 34));
        }
    }
    ...
    public String getDescription() {
        return Locale.getOrDefault("command.push.description", "Pushes a user 1 tile forward");
    }
