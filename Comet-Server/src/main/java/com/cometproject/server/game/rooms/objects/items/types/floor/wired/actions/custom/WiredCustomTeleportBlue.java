...
public class WiredCustomTeleportBlue extends WiredActionItem {
    ...
    public void onEventComplete(WiredItemEvent event) {
        ...
        for (final long itemId : this.getWiredData().getSelectedIds()) {
            ...
            final List<Integer> groupMembers = new ArrayList<>(this.getRoom().getGame().getTeams().get(GameTeam.BLUE));
            ...
            for (final Integer groupMember : groupMembers) {
                ...
                if (player.getPlayer().getEntity().getGameTeam().equals(GameTeam.BLUE)) {
