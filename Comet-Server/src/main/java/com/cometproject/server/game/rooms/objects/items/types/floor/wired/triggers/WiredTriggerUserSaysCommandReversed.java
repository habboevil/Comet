...
public class WiredTriggerUserSaysCommandReversed extends WiredTriggerItem {
    ...
    public static boolean executeTriggers(PlayerEntity playerEntity, String message) {
        ...
        if (getTriggers(playerEntity.getRoom(), WiredTriggerUserSaysCommandReversed.class).size() > 0) {
            ...
            for (WiredTriggerUserSaysCommandReversed floorItem : getTriggers(playerEntity.getRoom(), WiredTriggerUserSaysCommandReversed.class)) {
                ...
                if (!ownerOnly || isOwner) {
                    if (!floorItem.getWiredData().getText().isEmpty() && (executor.equals(values[0]) || executor.equals(":" + values[0]))) {
                        ...
                        if (Math.abs((posX - playerX)) <= limit && Math.abs(posY - playerY) <= limit) {
                            ...
                            playerEntity.getPlayer().getSession().send(new NotificationMessageComposer("generic", Locale.getOrDefault("command.user.toofar", "You are not at the established distance which is " + limit + " from %username%").replace("%username%", params[0])));
                            return true;
                        }
                    }
                }
