...
public class FilterWordsCommand extends ChatCommand {
    ...
    public void execute(Session client, String[] params) {
        if (params.length < 1) {
            client.getPlayer().getSession().send(new TalkMessageComposer(client.getPlayer().getEntity().getId(), Locale.getOrDefault("command.filter_words.incorrect_param", "incorrect parameter"), ChatEmotion.NONE, 34));
            return;
        }
    ...
    public String getDescription() {
        return Locale.getOrDefault("command.filter_words.description", "Adds a word to the hotel filter.");
    }
}
