...
public class DisconnectCommand extends ChatCommand {
    ...
    public void execute(Session client, String[] params) {
        ...
        if (session.getPlayer().getData().getRank() > client.getPlayer().getData().getRank()) {
            sendNotif(Locale.getOrDefault("command.disconnect.rank", "You cannot disconnect a person whit a higher rank."), client);
            return;
        }
        ...
        if (CometExternalSettings.enableStaffMessengerLogs) {
            this.logDesc = "-c disconnected user -d"
    ...
    public String getParameter() {
        return Locale.getOrDefault("command.parameter.username", "(kullanıcı)");
    }
