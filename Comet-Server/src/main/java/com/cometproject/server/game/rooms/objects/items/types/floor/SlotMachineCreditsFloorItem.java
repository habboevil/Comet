...
public class SlotMachineCreditsFloorItem extends RoomItemFloor {
    private boolean isInUse = false;

    public SlotMachineCreditsFloorItem(RoomItemData itemData, Room room) {
        super(itemData, room);
    }
    ...
    public boolean onInteract(RoomEntity entity, int requestData, boolean isWiredTrigger) {
        ...
        if (playerEntity.getPlayer().getData().getCredits() < playerEntity.getBetAmount() || playerEntity.getBetAmount() == 0) {
        ...
        playerEntity.getPlayer().getData().decreaseCredits(playerEntity.getBetAmount());
        ...
        if (random1 == random2 && random2 == random3) {
            ...
            playerEntity.getPlayer().getSession().send(new NotificationMessageComposer(image, "You have earned %q Coins with the slots.\n\n(%b x %m)"
                    .replace("%q", playerEntity.getBetAmount() * multiplier + "")
                    .replace("%b", playerEntity.getBetAmount() + "")
                    .replace("%m", multiplier + "")));

            playerEntity.getPlayer().getData().increaseCredits(playerEntity.getBetAmount() * multiplier);
        } else {
            playerEntity.getPlayer().getSession().send(
                    new NotificationMessageComposer("slot_loss", "Unfortunately the sequence was not equal and you lost %amount% coins.".replace("%amount%", playerEntity.getBetAmount() + ""))
            );
        }
