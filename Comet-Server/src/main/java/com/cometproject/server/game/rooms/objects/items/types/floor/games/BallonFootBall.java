...
public class BallonFootBall implements CometTask {
    ...
    private final boolean skipNext = false;
    private final boolean wasDribbling = false;
    private final int rollStage = -1;
...
//        items.put(item.getVirtualId(), item.getPosition().getZ());
        room.getEntities().broadcastMessage(new SlideObjectBundleMessageComposer(from.copy(), to.copy(), item.getVirtualId(), 0, Map.of(item.getVirtualId(), item.getPosition().getZ())));
    }
    ...
    private boolean isTileBocked(int x, int y) {
        ...
        return tile.getMovementNode() != RoomEntityMovementNode.OPEN || tile.getState() != RoomTileState.VALID;
