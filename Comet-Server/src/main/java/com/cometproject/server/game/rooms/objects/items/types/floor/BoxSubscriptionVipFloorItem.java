...
public class BoxSubscriptionVipFloorItem extends RoomItemFloor {
    ...
    public boolean onInteract(RoomEntity entity, int state, boolean isWiredTrigger) {
        ...
        if (hits < maxHits) {
            ...
            player.getSession().send(new TalkMessageComposer(((PlayerEntity) entity).getPlayer().getEntity().getId(), "You have opened the VIP subscription box and already have a current VIP subscription.", ChatEmotion.NONE, 34));
