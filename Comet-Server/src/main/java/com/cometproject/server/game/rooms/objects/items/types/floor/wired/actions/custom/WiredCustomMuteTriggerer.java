...
public class WiredCustomMuteTriggerer extends WiredActionItem {
    ...
    public void onEventComplete(WiredItemEvent event) {
        ...
        if (time > 0) {
            ((PlayerEntity) event.entity).getPlayer().getSession().send(new WhisperMessageComposer(((PlayerEntity) event.entity).getPlayerId(), "Wired Mute: Muted by " + time + (time > 1 ? "minutes" : "minute") + "! Message: " + (message == null || message.isEmpty() ? "No message" : message)));
