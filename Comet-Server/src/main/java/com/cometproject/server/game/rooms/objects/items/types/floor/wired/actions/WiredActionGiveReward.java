...
public class WiredActionGiveReward extends WiredActionItem {
    ...
    public void onEventComplete(WiredItemEvent event) {
        ...
        if (this.getWiredData().getParams().size() != 4 || !(event.entity instanceof final PlayerEntity playerEntity) || this.rewards.size() == 0)
            return;
        ...
        if (totalRewardLimit > 0 && this.totalRewardCounter >= totalRewardLimit) {
        ...
        if (canReceiveReward) {
            for (final Reward reward : this.rewards) {
                switch (howOften) {
                    ...
                    case MINUTES:
                        if (rewardTimings.get(this.getId()).containsKey(playerEntity.getPlayerId())) {
                            final long lastUserReward = rewardTimings.get(this.getId()).get(playerEntity.getPlayerId());
                    ...
                    case HOURS:
                        if (rewardTimings.get(this.getId()).containsKey(playerEntity.getPlayerId())) {
                            final long lastReward = rewardTimings.get(this.getId()).get(playerEntity.getPlayerId());
                ...
                if (probabilityBool) {
                    if (reward.isBadge) {
                        ...
                        final String[] itemData = reward.productCode.contains("%") ? reward.productCode.split("%") : reward.productCode.split(":");

                        if (isCurrencyReward(itemData[0])) {
                            final RewardType amount = getCurrencyTypeByKey(itemData[0]);
                            ...
                            switch (amount) {
                                case CREDITS:
                                    ...
                                    playerEntity.getPlayer().getSession().send(new AlertMessageComposer(
                                            Locale.getOrDefault("wired.reward.coins", "You have received %s coin(s)!").replace("%s", value + "")));
                                    break;

                                case ACTIVITY_POINTS:
                                    playerEntity.getPlayer().getData().increaseActivityPoints(value);
                                    playerEntity.getPlayer().getSession().send(new AlertMessageComposer(
                                            Locale.getOrDefault("wired.reward.duckets", "You have received %s ducket(s)!").replace("%s", value + "")));
                                    break;

                                case VIP_POINTS:
                                    playerEntity.getPlayer().getData().increaseVipPoints(value);
                                    playerEntity.getPlayer().getSession().send(new AlertMessageComposer(
                                            Locale.getOrDefault("wired.reward.diamonds", "You have received %s diamonds(s)!").replace("%s", value + "")));
                                    break;

                                case SEASONAL_POINTS:
                                    playerEntity.getPlayer().getData().increaseSeasonalPoints(value);
                                    playerEntity.getPlayer().getSession().send(new AlertMessageComposer(
                                            Locale.getOrDefault("wired.reward.seasonal", "You have received %s seasonal credit(s)!").replace("%s", value + "")));
                                    break;
                                ...
                                case ALERT:
                                    ...
                                    final String message = mergedParams.toString();
                            ...
                            final int itemId = Integer.parseInt(itemData[0]);
