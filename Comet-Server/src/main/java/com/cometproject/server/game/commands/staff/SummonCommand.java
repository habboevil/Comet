...
public class SummonCommand extends ChatCommand {
    ...
    public void execute(Session client, String[] params) {
        if (params.length != 1) {
            sendNotif(Locale.getOrDefault("command.summon.none", "Who do you want to pull?"), client);
            return;
        }
        ...
        if (username == client.getPlayer().getEntity().getUsername()) {
            sendNotif(Locale.getOrDefault("command.user.himself", "You cannot pull yourself."), client);
            return;
        }
