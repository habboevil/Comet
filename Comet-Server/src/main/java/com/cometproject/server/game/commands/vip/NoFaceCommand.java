...
public class NoFaceCommand extends ChatCommand {
    ...
    public void execute(Session client, String[] params) {
        final String figure = client.getPlayer().getData().getFigure();
        ...
        if (client.getPlayer().getData().getTemporaryFigure() != null) {
            ...
            if (figure.contains("hd-")) {
                final String[] head = ("hd-" + figure.split("hd-")[1].split("\\.")[0]).split("-");

                if (head.length < 2)
                    return;

                client.getPlayer().getData().setTemporaryFigure(figure);
                client.getPlayer().getData().setFigure(figure.replace(StringUtils.join(head, "-"), "hd-" + 99999 + "-" + (head.length != 3 ? head[head.length - 1] : head[2])));
            }
        }
    ...
    public String getDescription() {
        return Locale.getOrDefault("command.noface.description", "Make your user faceless");
    }
}
