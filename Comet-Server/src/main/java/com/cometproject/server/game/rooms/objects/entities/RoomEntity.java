...
public abstract class RoomEntity extends RoomFloorObject implements AvatarEntity {
    ...
    public void moveTo(int x, int y) {
        ...
        if (playerEntity != null && playerEntity.hasAttribute("playercontrol")) {
    ...
    public void findWalkPath(boolean firstGen, boolean again) {
        ...
        List<Square> path = EntityPathfinder.getInstance().makePath(this, new Position(walkingGoal.getX(), walkingGoal.getY()), this.getRoom().getData().getRoomDiagonalType().getKey(),
                false, firstGen);
        if (this.isFastWalkEnabled() && path.size() > 1) {
    ...
    public boolean positionToSetIsDoor() {
        if (this.positionToSet == null) return false;

        final RoomTile tile = this.getPositionToSetTile();

        if (tile == null) return false;

        return tile.isDoorTile();
    }

    public RoomTile getPositionToSetTile() {
        if (this.positionToSet == null) return null;

        return this.getRoom().getMapping().getTile(this.positionToSet.getX(), this.positionToSet.getY());
    }
    ...
    public boolean isIdleAndIncrement() {
        if (!(this instanceof PlayerEntity)) return this.isIdle;
        ...
        if (this.idleTime >= 600 && !this.isIdle) {
            this.isIdle = true;
    ...
    public boolean usingTeleportItem() {
        Object interacttpencours = this.getAttribute("interacttpencours");
        Object tptpencours = this.getAttribute("tptpencours");

        return interacttpencours != null && tptpencours != null;
    }
