...
public class PullCommand extends ChatCommand {
    ...
    public void execute(Session client, String[] params) {
        ...
        if (pulledEntity.getPosition().distanceTo(client.getPlayer().getEntity().getPosition()) != 2) {
            client.getPlayer().getSession().send(new WhisperMessageComposer(client.getPlayer().getEntity().getId(), Locale.getOrDefault("command.notaround", "Oeps! %playername% is not nearby, walk to this player.").replace("%playername%", pulledEntity.getUsername()), 34));
            return;
        }
    ...
    public String getDescription() {
        return Locale.getOrDefault("command.pull.description", "Pulls a user in front of you");
    }
