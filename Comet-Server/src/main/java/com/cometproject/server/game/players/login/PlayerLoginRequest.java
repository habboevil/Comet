...
public class PlayerLoginRequest implements CometTask {
    ...
    private final Logger log = LogManager.getLogger(PlayerLoginRequest.class.getName());
    ...
    public void run() {
        ...
        try {
            ...
            log.info("Player {} Logged in with MachineID {}", client.getPlayer().getData().getUsername(), this.client.getUniqueId());

            if (!Comet.isDebugging && (this.client.getUniqueId() == null || this.client.getUniqueId().length() == 0)) {
                log.info("A invalid MachineID was used to login");
                client.disconnect();
                return;
            }
            ...
            if (hasTradeBan)
                client.sendQueue(new NotificationMessageComposer("trade_block", Locale.getOrDefault("user.got.tradeblocked", "Suspicious activity has been detected on your account and your trades have been blocked.")));


            if (CometSettings.motdEnabled) {
                client.sendQueue(new OpenLinkMessageComposer("habbopages/welcome.txt?" + Comet.getTime()));
            }
            ...
            for (final IMessengerFriend friend : client.getPlayer().getMessenger().getFriends().values()) {
                ...
                friend.getSession().send(new NotificationMessageComposer("usr/look/" + player.getData().getFigure(),
                        Locale.getOrDefault("player.online", "%username% is connected!")
            ...
            if (client.getPlayer().getSettings().getNuxStatus() == 0 && CometExternalSettings.enableStaffMessengerLogs) {
                for (Session staff : ModerationManager.getInstance().getModerators()) {
                    staff.sendQueue(new InviteFriendMessageComposer(Locale.getOrDefault("onboarding.alert", "%p just registered in " + CometSettings.hotelName).replace("%p", client.getPlayer().getData().getUsername()), Integer.MIN_VALUE + 5000));
                }
            }
            ...
            if (SessionManager.isLocked) {
                client.sendQueue(new AlertMessageComposer("Hotel is closed, come back soon!"));
