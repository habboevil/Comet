...
public class WarpCommand extends ChatCommand {
    ...
    public void execute(Session client, String[] params) {
        ...
        if ((room.getData().getOwnerId() != client.getPlayer().getId() && !client.getPlayer().getPermissions().getRank().roomFullControl())) {
            sendNotif(Locale.getOrDefault("command.need.rights", "You have no rights to use this command in the bedroom."), client);
            return;
        }

        if (params.length != 1) {
            sendNotif(Locale.getOrDefault("command.warp.none", "Who do you want remove from here?"), client);
            return;
        }
