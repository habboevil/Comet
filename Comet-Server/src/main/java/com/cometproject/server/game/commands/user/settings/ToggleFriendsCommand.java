...
public class ToggleFriendsCommand extends ChatCommand {
    ...
    public void execute(Session client, String[] params) {
        if (client.getPlayer().getSettings().getAllowFriendRequests()) {
            ...
            sendNotif(Locale.getOrDefault("command.togglefriends.disabled", "You've turned off friend requests."), client);
        } else {
            client.getPlayer().getSettings().setAllowFriendRequests(true);
            sendNotif(Locale.getOrDefault("command.togglefriends.enabled", "You've turned on friend requests."), client);
    ...
    public String getDescription() {
        return Locale.getOrDefault("command.togglefriends.description", "Turn friend requests on or off");
    }
}
