...
public class EnableCommand extends ChatCommand {
    ...
    public void execute(Session client, String[] params) {
        ...
        if ((room.getData().getOwnerId() != client.getPlayer().getId() && !client.getPlayer().getPermissions().getRank().roomFullControl())) {
            sendNotif(Locale.getOrDefault("command.need.rights", "You do not have rights to use the command in this room."), client);
            return;
        }
        ...
        final String disabledCommand = params[0];
    ...
    public String getDescription() {
        return Locale.getOrDefault("command.enablecommand.description", "Activate a enable command in your room");
    }
}
