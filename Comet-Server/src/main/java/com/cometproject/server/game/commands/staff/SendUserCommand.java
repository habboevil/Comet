...
public class SendUserCommand extends ChatCommand {
    ...
    public void execute(Session client, String[] params) {
        ...
        if (session == null) {
            client.getPlayer().getSession().send(new TalkMessageComposer(-1, Locale.getOrDefault("command.send_user.user_not_found", "This user is offline."), ChatEmotion.NONE, 1));
            return;
        }
    ...
    public String getParameter() {
        return Locale.getOrDefault("command.send_user.parameters", "(room id) (user)");
    }
