...
public class CloneRoomCommand extends ChatCommand {
    ...
    public void execute(Session client, String[] params) {
        ...
        try {
            ...
            final int roomId = RoomManager.getInstance().createRoom(bundle.getConfig().getRoomName().replace("%username%", client.getPlayer().getData().getUsername()), "", bundle.getRoomModelData(), 0, 20, 0, creationTime, client, bundle.getConfig().getThicknessWall(), bundle.getConfig().getThicknessFloor(), bundle.getConfig().getDecorations(), bundle.getConfig().isHideWalls());

            final Set<IRoomItemData> itemData = Sets.newHashSet();
            final int playerId = client.getPlayer().getId();

            for (final RoomBundleItem roomBundleItem : bundle.getRoomBundleData()) {
            ...
            this.logDesc = "%s, '%b' copied the room"
                    .replace("%s", client.getPlayer().getData().getUsername())
                    .replace("%b", client.getPlayer().getEntity().getRoom().getData().getName());

        } catch (Exception e) {
            client.send(new MotdNotificationMessageComposer("Invalid room package details, contact an administrator."));
            client.send(new BoughtItemMessageComposer(BoughtItemMessageComposer.PurchaseType.BADGE));
        }
    }
