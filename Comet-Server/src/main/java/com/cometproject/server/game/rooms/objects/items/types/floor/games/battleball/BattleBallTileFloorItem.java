...
public class BattleBallTileFloorItem extends RoomItemFloor {
    ...
    private static List<BattleBallTileFloorItem> buildBanzaiRectangle(BattleBallTileFloorItem triggerItem, int x, int y, int goX, int goY, int currentDirection, int turns, GameTeam team) {
        ...
        for (int i = 0; i < 4; ++i) {
            ...
            if (room.getMapping().getTile(nextX, nextY) == null || (obj = room.getItems().getFloorItem(room.getMapping().getTile(nextX, nextY).getTopItem())) == null || !(obj instanceof BattleBallTileFloorItem) || (item = (BattleBallTileFloorItem) obj).getTeam() != team || item.getPoints() != 3)
                continue;
        ...
        if (this.points == 3) {
            ...
            if (rectangle != null) {
                for (final BattleBallTileFloorItem roomItemFloor : this.getRoom().getItems().getByClass(BattleBallTileFloorItem.class)) {
                    ...
                    if (!borderCheck[0] || !borderCheck[1] || !borderCheck[2] || !borderCheck[3] || roomItemFloor.getId() == this.getId())
                        continue;
