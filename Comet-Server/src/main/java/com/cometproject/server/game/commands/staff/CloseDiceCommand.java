...
public class CloseDiceCommand extends ChatCommand {
    ...
    public void execute(Session client, String[] params) {
        ...
        for (final RoomItemFloor item : playerEntity.getRoom().getItems().getByClass(DiceFloorItem.class)) {
        ...
        if (diceAmount > 0) {
            String filteredMessage = Locale.getOrDefault("command.closedice.message", "* %username% closed %amount% dice(s) *").replace("%username%", playerEntity.getUsername()).replace("%amount%", "" + diceAmount);
    ...
    public String getDescription() {
        return Locale.getOrDefault("command.closedice.description", "Encloses the dice(s) around the user using the command.");
    }
}