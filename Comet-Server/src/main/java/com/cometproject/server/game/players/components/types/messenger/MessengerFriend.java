...
public class MessengerFriend implements IMessengerFriend {
    ...
    public boolean isInRoom() {
        ...
        return !client.getPlayer().getSettings().getHideInRoom() && client.getPlayer().getSettings().getAllowFollow();
    }
    ...
    public boolean isOnline() {
        final boolean isRealOnline = PlayerManager.getInstance().isOnline(userId);

        if (!isRealOnline) {
            return false;
        }

        final Session playerSession = NetworkManager.getInstance().getSessions().getByPlayerId(userId);

        if (playerSession == null || playerSession.getPlayer() == null) {
            return false;
        }

        return !playerSession.getPlayer().getSettings().getHideOnline();
    }
