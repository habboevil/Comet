...
public class BoxDiamondsFloorItem extends RoomItemFloor {
    ...
    public boolean onInteract(RoomEntity entity, int state, boolean isWiredTrigger) {
        ...
        if (hits < maxHits) {
            ...
            ((PlayerEntity) entity).getPlayer().getSession().send(new TalkMessageComposer(((PlayerEntity) entity).getPlayer().getEntity().getId(), "You have opened the diamond box and obtained " + result + " diamonds", ChatEmotion.NONE, 34));
