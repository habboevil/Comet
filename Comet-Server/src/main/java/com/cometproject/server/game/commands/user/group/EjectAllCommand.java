...
public class EjectAllCommand extends ChatCommand {
    ...
    public void execute(Session client, String[] params) {
        ...
        if (room.getData().getOwnerId() != client.getPlayer().getId()) {
            ...
            for (final RoomItem item : itemsToRemove) {
                if (item instanceof RoomItemFloor) {
                    ...
                    client.getPlayer().getEntity().getRoom().getItems().removeItem(((RoomItemWall) item), client, true, true);
                }
            }
        } else {
                ...
                if (groupMemberSession != null && groupMemberSession.getPlayer() != null && group != null) {
                    ...
                    for (final RoomItem item : floorItemsOwnedByPlayer) {
                        if (item instanceof RoomItemFloor) {
                            ...
                            client.getPlayer().getEntity().getRoom().getItems().removeItem(((RoomItemWall) item), client, false, true);
                        }
                    }
                }
        ...
        this.logDesc = "User %s gave ejectall in room '%b', whose owner is %c"
    ...
    public String getDescription() {
        return Locale.getOrDefault("command.ejectall.description", "Ejects all the furniture from the room.");
    }
