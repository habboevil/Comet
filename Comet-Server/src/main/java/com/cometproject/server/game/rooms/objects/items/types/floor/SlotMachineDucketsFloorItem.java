...
public class SlotMachineDucketsFloorItem extends RoomItemFloor {
    ...
    public boolean onInteract(RoomEntity entity, int requestData, boolean isWiredTrigger) {
        ...
        if (random1 == random2 && random2 == random3) {
            ...
            playerEntity.getPlayer().getSession().send(new NotificationMessageComposer(image, "You won %q Duckets with the slots.\n\n(%b x %m)"
            ...
            playerEntity.getPlayer().getSession().send(
                    new NotificationMessageComposer("slot_loss", "Unfortunately the sequence was not the same and you lost %amount% duckets.".replace("%amount%", playerEntity.getBetAmount() + ""))
            );
        }
        ...
        playerEntity.getPlayer().getSession().send(new WhisperMessageComposer(this.getVirtualId(), "You took it out " + rand1 + " " + rand2 + " " + rand3 + ".", 34));
