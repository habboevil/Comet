...
public abstract class RoomItem extends BigRoomFloorObject implements Attributable {
    ...
    public RoomItem(int id, int userId, PlayerItem item, String extradata, int limitedStack, int limitedSells) {
    ...
    public String getExtraData() {
        if (this instanceof WiredFloorItem) {
            return ((WiredFloorItem) this).getState() ? "1" : "0";
        }

        if (this instanceof SoundMachineFloorItem) {
            return ((SoundMachineFloorItem) this).getState() ? "1" : "0";
        }
    ...
    public boolean isUsable() {
        return this.getDefinition().getInteractionCycleCount() > 1;
    }
