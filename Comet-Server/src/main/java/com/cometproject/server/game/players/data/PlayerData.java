...
public class PlayerData implements IPlayerData {
    ...
    public PlayerData(ResultSet data, Player player) throws SQLException {
        this(data.getInt("playerId"),
                ...
                data.getString("playerData_machineID"),
                player);
...
//        }
    ...
    public void saveNow() {
        PlayerDao.updatePlayerData(id, username, rank, motto, figure, credits, vipPoints, gender, favouriteGroup, activityPoints, seasonalPoints, questId, achievementPoints, xpPoints, nameColour, tag, emojiEnabled, gamesWin, bonusPoints, endVipTimestamp, snowXp, kisses, machineID);
    }
    ...
    public int getEndVipTimestamp() {
        return this.endVipTimestamp;
    }
    ...
    @Override
    public void setKisses(int amount) {
        this.kisses = amount;
        flush();
    }

    @Override
    public void increaseKisses(int amount) {
        this.kisses += amount;
    }

    @Override
    public void incrementKisses() {
        this.kisses++;
    }

    @Override
    public void decreaseKisses(int amount) {
        this.kisses -= amount;
        flush();
    }
    ...
    public PlayerData clone() {
        ...
        try {
            ...
            data = new PlayerData(this.id, this.username, this.motto, this.figure, this.gender, this.email, this.rank, this.credits, this.vipPoints, this.activityPoints, this.seasonalPoints, this.regDate, this.lastVisit, this.vip, this.achievementPoints, this.xpPoints, this.regTimestamp, this.favouriteGroup, this.ipAddress, this.questId, this.timeMuted, this.nameColour, this.tag, this.emojiEnabled, this.gamesWin, this.bonusPoints, this.endVipTimestamp, this.snowXp, this.kisses, this.machineID, this.player);
        }
        return data;
    }
