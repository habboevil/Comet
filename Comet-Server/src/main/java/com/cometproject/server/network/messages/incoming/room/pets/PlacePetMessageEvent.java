...
public class PlacePetMessageEvent implements Event {
    ...
    public void handle(Session client, MessageEvent msg) throws Exception {
        ...
        if (room.getEntities().getPetEntities().size() >= 10) {
        ...
        if (isOwner || room.getData().isAllowPets() || client.getPlayer().getPermissions().getRank().roomFullControl()) {
            if (pet == null) {
                return;
            }
            ...
            PetEntity petEntity = pet instanceof PetMonsterPlantData ? room.getPets().addMonsterPlant((PetMonsterPlantData) pet, position) : room.getPets().addPet(pet, position);
            ...
            if (client.getPlayer().getId() != room.getData().getOwnerId() && pet.getTypeId() == PetType.HORSE) {
