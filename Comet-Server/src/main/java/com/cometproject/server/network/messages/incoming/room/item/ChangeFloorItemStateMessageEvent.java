...
public class ChangeFloorItemStateMessageEvent implements Event {
    public void handle(Session client, MessageEvent msg) {
        ...
        if (client.getPlayer().getEntity().hasRights() && client.getPlayer().getEntity().getBuildingType().equals(BuildingType.FILL)) {
            if (client.getPlayer().getEntity().hasAttribute("fill_command_item")) {
                ...
                if (firstItem.getId() == item.getId()) {
                    ...
                    client.send(new WhisperMessageComposer(client.getPlayer().getEntity().getId(), Locale.getOrDefault("fill.command.area.undo", "Este furni ya no ha sido seleccionado para ser llenado por área.")));
                    return;
                }

                if (firstItem.getDefinition().getId() != item.getDefinition().getId()) {
                    client.getPlayer().getEntity().removeAttribute("fill_command_item");
                    client.send(new WhisperMessageComposer(client.getPlayer().getEntity().getId(), Locale.getOrDefault("fill.command.area.incompatible.selection", "Sólo puedes seleccionar furnis si son iguales.")));
                    return;
                }
                ...
                client.getPlayer().getEntity().removeAttribute("fill_command_item");
                client.send(new WhisperMessageComposer(client.getPlayer().getEntity().getId(), Locale.getOrDefault("fill.command.area.done", "¡Área llenada exitosamente!")));
            } else {
                client.getPlayer().getEntity().setAttribute("fill_command_item", item);
                client.send(new WhisperMessageComposer(client.getPlayer().getEntity().getId(), Locale.getOrDefault("fill.command.area.first_item.selected", "Has seleccionado el primer elemento, el ancla de selección para el llenado por área. ¡Para llenar, haz clic 2 veces en otro furni igual!")));
            }
        ...
        if (client.getPlayer().viewingHeight() && client.getPlayer().getEntity() != null) {
            client.send(
                    new WhisperMessageComposer(client.getPlayer().getEntity().getId(), "La altura del furni es: " + item.getPosition().getZ())
            );
        ...
        if (client.getPlayer().getIsFurniturePickup()) {
            ...
            client.send(new NotificationMessageComposer(
                    "generic",
                    Locale.getOrDefault("command.pickup.success", "Fueron recolectados %count% furnis.").replace("%count%", count + "")
            ));
            return;
        }
