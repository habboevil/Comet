...
public class WhisperMessageEvent implements Event {
    public void handle(Session client, MessageEvent msg) {
            ...
            for (String username : client.getPlayer().getGroupWhispers()) {
                ...
                if (!client.getPlayer().getPermissions().getRank().roomFilterBypass()) {
                    ...
                    if (filterResult.isBlocked()) {
                        ...
                        client.getLogger().info(("The filter detected a forbidden word in the message: \"" + message + "\""));
                        continue;
                    }
                ...
                whisperedPlayer.getSession().send(new WhisperMessageComposer(whisperedPlayer.getId(), "<b>" + client.getPlayer().getData().getUsername() + "</b>, told the group: " + filteredMessage2, 1));
            }
            client.send(new WhisperMessageComposer(playerEntity.getId(), "<b>" + client.getPlayer().getData().getUsername() + "</b>, he told the group " + message, bubbleId));
        ...
        if (userTo.getPlayer().getSettings().disableWhisper() && !(client.getPlayer().getData().getRank() > Integer.parseInt(Locale.getOrDefault("whisper.minrank.force", "5")))) {
            client.send(new WhisperMessageComposer(client.getPlayer().getId(), Locale.getOrDefault("whisper.disabled", "Ops! The user you tried to message has whispers disabled!")));
            return;
        }
