...
public class CreateRoomMessageEvent implements Event {
    public void handle(Session client, MessageEvent msg) {
        ...
        if (lastRoomCreatedDifference < 30) {
            client.send(new MotdNotificationMessageComposer(Locale.getOrDefault("room.creation.time", "You can create only one room every 30 seconds. (You still have " + (30 - lastRoomCreatedDifference) + " seconds remaining).")));
            return;
        }
