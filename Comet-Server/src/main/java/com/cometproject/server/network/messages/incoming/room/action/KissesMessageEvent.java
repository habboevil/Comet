...
public class KissesMessageEvent implements Event {
    ...
    public void handle(Session client, MessageEvent msg) throws Exception {
        ...
        if (actorPosition.distanceTo(user.getPlayer().getEntity().getPosition()) > 1.0) {
            client.send(new NotificationMessageComposer("action_distance_error", Locale.getOrDefault("kisses.far", "You must be in front of the person to perform the action.")));
            return;
        }
        ...
        if (CometSettings.useNewKissSystem) {
            if (client.getPlayer().antiSpam("SentKissesToUser", 1.5)) {
                client.send(new NotificationMessageComposer("generic", Locale.getOrDefault("kisses.sent.too.fast", "You're sending kisses too fast.")));
                return;
            }
            ...
            room.getEntities().broadcastMessage(
                    new TalkMessageComposer(
                            ...
                            Locale.getOrDefault("user.kiss.action", "%user_one% kissed %user_two%.").replace("%user_one%", client.getPlayer().getData().getUsername()).replace("%user_two%", user.getPlayer().getData().getUsername()),
                            ChatEmotion.NONE, 42)
            );
        } else {
