...
//        }
    ...
    public void broadcastHotelAlert(IMessageComposer msg) {
        for (ISession session : this.sessions.values()) {
            if (session.getPlayer() != null && session.getPlayer().getSettings() != null && !session.getPlayer().getSettings().disableHotelAlert()) {
                session.send(msg);
            }
        }
    }

    public void broadcastHotelAlertLink(IMessageComposer msg) {
        for (ISession session : this.sessions.values()) {
            if (session.getPlayer() != null && session.getPlayer().getSettings() != null && !session.getPlayer().getSettings().disableHotelAlertLinks()) {
                session.send(msg);
            }
        }
    }
