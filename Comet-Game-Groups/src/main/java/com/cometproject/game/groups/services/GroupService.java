...
public class GroupService implements IGroupService {
    ...
    @Override
    public Set<Integer> getActiveForums() {
        return this.groupRepository.getActiveGroups();
    }

    @Override
    public Set<Integer> getMostViewed() {
        return this.groupRepository.getMostViewed();
    }
    ...
    public void addGroupMember(IGroup group, IGroupMember groupMember) {
        if (groupMember.getMembershipId() == 0) {
            this.groupMemberRepository.create(group.getId(), groupMember.getPlayerId(), groupMember.getAccessLevel(), (member) -> groupMember.setMembershipId(member.getMembershipId()));
        }
