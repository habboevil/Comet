...
public class MySQLGroupRepository extends MySQLRepository implements IGroupRepository {
    ...
    public void getGroupIdsByPlayerId(final int playerId, Consumer<List<Integer>> consumer) {
        ...
        select("SELECT DISTINCT `group_id` FROM group_memberships WHERE player_id = ?;", (data -> groups.add((data.readInteger(1)))), playerId);
    ...
    public Set<Integer> getActiveGroups() {
        final Set<Integer> groupsId = Sets.newHashSet();

        select("SELECT `group_id`, COUNT(*) AS `group_message_counts` FROM group_forum_messages WHERE `author_timestamp` > ? GROUP BY `group_id` ORDER BY `group_message_counts` DESC LIMIT 100", (data -> {
            groupsId.add(data.readInteger("group_id"));
        }), (System.currentTimeMillis() / 1000L - 7 * 24 * 60 * 60));

        return groupsId;
    }

    @Override
    public Set<Integer> getMostViewed() {
        final Set<Integer> groupsId = Sets.newHashSet();
