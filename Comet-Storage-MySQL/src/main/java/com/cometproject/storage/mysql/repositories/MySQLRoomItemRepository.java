...
public class MySQLRoomItemRepository extends MySQLRepository implements IRoomItemRepository {
    ...
    public void getRoomIdByItemId(long itemId, Consumer<Integer> idConsumer) {
        select("SELECT `room_id` FROM items WHERE id = ? LIMIT 1;", (data) -> idConsumer.accept(data.readInteger("room_id")), itemId);
    }
    ...
    public void createItem(int playerId, int itemId, String data, Consumer<Long> idConsumer) {
        insert("INSERT INTO items (`user_id`, `room_id`, `base_item`, `extra_data`, `x`, `y`, `z`, `rot`, `wall_pos`) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?);", (key) -> idConsumer.accept(key.readLong(1)), playerId, 0, itemId, data, 0, 0, 0, 0, "");
    }

    @Override
    public void purchaseItems(List<CatalogPurchase> purchases, Consumer<List<Long>> idConsumer, int viewingUserId, ICatalogItem item) {
        final List<Long> itemIds = Lists.newArrayList();

        insertBatch("INSERT INTO items (`user_id`, `room_id`, `base_item`, `extra_data`, `x`, `y`, `z`, `rot`, `wall_pos`, `limited_data`) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?);", (stmt) -> {
            for (CatalogPurchase catalogPurchase : purchases) {
                ...
                if (item.getLimitedTotal() > 0) {
                    stmt.setString(10, item.getLimitedSells() + ":" + item.getLimitedTotal());
                } else {
